                    ...
                    if noise_probability < 0 or noise_probability > 1:
                        ...
                        with st.spinner('Processing image...'):
                            ...
                            with open(selected_image_path, "rb") as f:
                                ...
                                salt_color_encoded = urllib.parse.quote(salt_color)
                                pepper_color_encoded = urllib.parse.quote(pepper_color)

                                response = requests.post(
                                    f"{ENDPOINT_ADD_NOISE}?noise_prob={noise_probability}&salt_color={salt_color_encoded}&pepper_color={pepper_color_encoded}",
                                    files=files
                                )

                                st.write(f"Salt color sent to backend: {salt_color}")
                                st.write(f"Pepper color sent to backend: {pepper_color}")
...
# ---------------------- RGB Extraction and Image Operations Section ---------------------- #
    ...
    if uploaded_file:
        ...
        st.subheader("Tambahan Fitur Untuk Perbaikan")
        operation = st.selectbox("Choose an operation", [
            "Convolution", "Zero Padding", "Filter", "Fourier Transform", "Reduce Periodic Noise"
        ])
        
        if operation == "Convolution":
            kernel_type = st.selectbox("Kernel Type", ["sharpen", "average", "edge_detection"])
            if st.button("Apply Operation"):
                try:
                    # Reset pointer sebelum membaca ulang file
                    uploaded_file.seek(0)
                    
                    # Siapkan file untuk dikirim
                    files = {
                        "file": ("image.jpg", uploaded_file, uploaded_file.type)  # Sertakan nama file dan MIME type
                    }
                    params = {"kernel_type": kernel_type}

                    # Kirim request POST ke backend
                    response = requests.post(f"{BACKEND_URL}/convolution", files=files, params=params)
        ...
        elif operation == "Zero Padding":
            padding_size = st.slider("Padding Size", 1, 50, 10)
            if st.button("Apply Operation"):
                try:
                    # Reset the file pointer
                    uploaded_file.seek(0)
                    files = {
                        "file": ("image.jpg", io.BytesIO(uploaded_file.read()), "image/jpeg")
                    }
                    response = requests.post(
                        f"{BACKEND_URL}/zero_padding",
                        files=files,
            ...
            if st.button("Apply Operation"):
                try:
                    ...
                    response = requests.post(
                        f"{BACKEND_URL}/fourier_transform",
                        files=files
                    )
                    if response.status_code == 200:
                        display_image(response.content)
        ...
        elif operation == "Reduce Periodic Noise":
            if st.button("Apply Operation"):
                try:
                    # Reset the file pointer before reading
                    uploaded_file.seek(0)
                    ...
                    response = requests.post(
                        f"{BACKEND_URL}/reduce_periodic_noise",
                        files=files
                    )
                    if response.status_code == 200:
                        display_image(response.content)
        ...
        if st.button("Apply Arithmetic Operation"):
            if operation != "None":
                ...
                if processed_img is not None:
                    st.image(processed_img, caption=f"Image After {operation} Operation", channels="BGR", use_container_width=True)
            else:
        ...
        if logic_operation in ["AND", "XOR"]:
            ...
            if image2_file:
                st.image(image2_file, caption="Second Image", use_container_width=True)
                image2 = image2_file
        ...
        if st.button("Apply Logic Operation"):
            if logic_operation != "None":
                if logic_operation in ["AND", "XOR"] and not image2:
                    ...
                    if processed_img is not None:
                        st.image(processed_img, caption=f"Image After {logic_operation} Operation", channels="BGR", use_container_width=True)
            else:
        ...
        if st.button("Generate Histograms"):
            ...
            if grayscale_hist_url and color_hist_url:
                ...
                st.image(grayscale_hist_url, caption="Grayscale Histogram", use_container_width=True)
                st.image(color_hist_url, caption="Color Histogram", use_container_width=True)
        ...
        if st.button("Equalize Histogram"):
            ...
            if equalized_img is not None:
                st.image(equalized_img, caption="Equalized Image", channels="BGR", use_container_width=True)
        ...
        if st.button("Apply Histogram Specification"):
            if not reference_file:
                ...
                if specified_img is not None:
                    st.image(specified_img, caption="Specified Histogram Image", channels="BGR", use_container_width=True)
